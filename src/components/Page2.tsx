...
const ScrollImageEffect = () => {
  ...
  useGSAP(() => {
    ...
    // Image animations
    [img1Ref, img2Ref, img3Ref].forEach((ref, index) => {

      const delay = index * 0.2;

      // Fade-in effect
      mainTl.to(ref.current, { opacity: 1, duration: 0.3 }, `+=${delay}`);

      // 3D translation effect
      mainTl.to(
        ref.current,
        {
          duration: 0.5,
          ease: "power2.inOut",
          onUpdate: function () {
            const prog = globalTl.progress();
            const newZ = -300 + 80 * (prog * 10); // Adjusted formula

            [img1Ref, img2Ref, img3Ref].forEach((imageRef, i) => {
              if (i <= index) {
                imageRef.current!.style.transform = `translate3d(0px, 0px, ${newZ}px) scale(${0.5 + 0.5 * prog})`;
              }
            });
          },
        },
  ...
  return (
    ...
    <section ref={maincontainerRef} className="transition-colors duration-500 overflow-x-hidden relative w-full h-full">
      ...
      <div className="goal-slide w-full h-[100dvh] bg-white absolute top-0" ref={containerRef} style={{ perspective: "100dvh" }}>
        <button
      ...
      <div className=" Thoughts-slide w-full h-screen overflow-hidden relative top-0 bg-slate-500">
        <button className="absolute font-Satoshi text-white font-bold top-[44%] left-[50%] translate-x-[-50%] translate-y-[-40%] px-6 py-2 text-sm md:text-xl rounded-full bg-gradient-to-r from-[#FF7676] via-[#2C4CFF] to-[#FF4000]">
          <span className="text-content">Amplify your Thoughts</span>
        </button>

        <div className="absolute  overflow-hidden rounded-xl w-[350px] h-48 left-[65%] top-20 hidden lg:block" >
          <Image
            src="/img/slide1/2.png"
            fill
            alt="2"
        ...
        <div className="rounded-lg  absolute overflow-hidden w-[160px] h-56 left-[25%] top-72 hidden lg:block">
          <Image
        ...
        <div className="w-28 h-20 rounded-lg overflow-hidden absolute top-96 left-[55%] hidden lg:block">
          <Image
